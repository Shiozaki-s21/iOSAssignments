...
class TodosTableViewController: UITableViewController {
  ...
  var todoEntities:[TodoEntity] = [TodoEntity]()
  ...
  func fetchCoreData() {
    ...
    do {
      ...
      self.todoEntities = entites
      todoList.removeAll()
      for entity in todoEntities {
        let todoItem = TodoItem()
        todoItem.text = entity.value(forKey: "text") as! String
        todoItem.checked = entity.value(forKey: "checked") as! Bool
        let priority:Int = entity.value(forKey: "priority") as! Int
        

        switch priority {
          case 0:
            todoList.addTodo(item: todoItem, for: TodoList.Priority.low )
          case 1:
            todoList.addTodo(item: todoItem, for: TodoList.Priority.medium )
          case 2:
            todoList.addTodo(item: todoItem, for: TodoList.Priority.high )
          default:
            todoList.addTodo(item: todoItem, for: TodoList.Priority.medium )
        }
  ...
  func deleteTodoItemsFromCore(item:TodoItem) {
    CoreDataManager.shared.deleteContext(target: item.text)
  }
      ...
      if var selectedRows = tableView.indexPathsForSelectedRows {
        ...
        for indexPath in selectedRows {
          if let priority = priorityForSectionIndex(indexPath.section) {
            ...
            deleteTodoItemsFromCore(item: item)
          }
        }
  ...
  extension TodosTableViewController: AddItemViewControllerDelegate {
    ...
    func addItemDidFinishAdding(_ entity: TodoEntity) {
      navigationController?.popViewController(animated: true)
      
      let item = TodoItem()
      item.text = entity.value(forKey: "text") as! String
      item.checked = entity.value(forKey: "checked") as! Bool
